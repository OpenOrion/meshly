...
class ArrayMetadata(BaseModel):
    ...
    filename: Optional[str] = Field(
        None, description="Filename for the array data (used in multi-array zip files)")
...
class MeshMetadata(BaseModel):
    ...
    index_count: Optional[int] = Field(
        None, description="Number of indices in the mesh")


class ArraysMetadata(RootModel):
    """Container for multiple array metadata entries."""
    root: Dict[str, ArrayMetadata] = Field(
        ..., description="Dictionary mapping array names to their metadata")
...
def save_combined_data_to_zip(
    ...
    try:
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            ...
            zipf.writestr(f"{mesh_dir}/metadata.json",
                          mesh_metadata.model_dump_json(indent=2))
            ...
            zipf.writestr(f"{arrays_dir}/metadata.json",
                          arrays_metadata.model_dump_json(indent=2))
...
def load_combined_data(
    destination: Union[str, bytes, io.BytesIO],
    mesh_dir: str = "mesh",
    arrays_dir: str = "arrays"
    ...
    Args:
        destination: Path to the input zip file, bytes object containing zip data,
    ...
    try:
        ...
        if isinstance(destination, str):
            if not os.path.exists(destination):
                raise FileNotFoundError(f"Zip file not found: {destination}")
            zip_source = destination
        elif isinstance(destination, bytes):
            zip_source = io.BytesIO(destination)
        elif isinstance(destination, io.BytesIO):
            zip_source = destination
        else:
            raise TypeError(f"Unsupported zip_path type: {type(destination)}")
        ...
        with zipfile.ZipFile(zip_source, 'r') as zipf:
            ...
            mesh_metadata = MeshMetadata.model_validate_json(
                zipf.read(f"{mesh_dir}/metadata.json"))
            ...
            # Load array metadata
            arrays_metadata = ArraysMetadata.model_validate_json(
                zipf.read(f"{arrays_dir}/metadata.json"))
        ...
        raise ValueError(f"Invalid zip file: {destination}")
