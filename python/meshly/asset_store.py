...
Example:
    ...
    # Save a packable
    my_mesh.save_to_store(store, "abc123/result")
    
    # Load from store
    loaded = Mesh.load_from_store(store, "abc123/result")
"""
...
class AssetStore:
    ...
    def save_extracted(
        ...
        """Save an ExtractedPackable to the store.
        
        Saves all assets to the assets directory and metadata (data.json + schema.json)
        to the specified path.
        
        Args:
            extracted: The extracted packable data
            path: Relative path for the metadata (e.g., 'abc123/result')
        ...
        for asset_checksum, asset_bytes in extracted.assets.items():
            self.save_asset(asset_bytes, asset_checksum)
        
        # Create metadata directory
        meta_dir = self.metadata_dir(path)
        meta_dir.mkdir(parents=True, exist_ok=True)
        
        # Save data.json
        data_path = meta_dir / self.DATA_FILE
        ...
        if extracted.metadata.json_schema:
            schema_path = meta_dir / self.SCHEMA_FILE
