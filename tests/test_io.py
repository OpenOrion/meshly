...
class TestIO(unittest.TestCase):
    ...
    def test_save_load_file(self):
        ...
        print(
            f"File compression ratio: {os.path.getsize(file_path) / large_array.nbytes:.2f}")
    ...
    def test_save_load_array_zip(self):
        ...
        # Print compression ratio
        print(
            f"Zip compression ratio: {os.path.getsize(zip_path) / large_array.nbytes:.2f}")
    ...
    def test_save_load_multiple_arrays(self):
        ...
        # Print compression ratio
        print(
            f"Multiple arrays compression ratio: {os.path.getsize(zip_path) / total_size:.2f}")
    ...
    def test_save_load_encoded_mesh(self):
        ...
        self.assertEqual(loaded_encoded_mesh.vertex_count,
                         self.encoded_mesh.vertex_count)
        self.assertEqual(loaded_encoded_mesh.vertex_size,
                         self.encoded_mesh.vertex_size)
        self.assertEqual(loaded_encoded_mesh.index_count,
                         self.encoded_mesh.index_count)
        self.assertEqual(loaded_encoded_mesh.index_size,
                         self.encoded_mesh.index_size)
        self.assertEqual(loaded_encoded_mesh.vertices,
                         self.encoded_mesh.vertices)
        self.assertEqual(loaded_encoded_mesh.indices,
                         self.encoded_mesh.indices)
        ...
        decoded_triangles = self.get_triangles_set(
            decoded_vertices, decoded_indices)
    ...
    def test_save_load_mesh(self):
        ...
        np.testing.assert_array_almost_equal(
            loaded_mesh.vertices, self.mesh.vertices)

        # Check that the triangles match
        original_triangles = self.get_triangles_set(
            self.mesh.vertices, self.mesh.indices)
        loaded_triangles = self.get_triangles_set(
            loaded_mesh.vertices, loaded_mesh.indices)
    ...
    def test_save_load_combined_data(self):
        ...
        loaded_mesh, loaded_arrays, loaded_metadata = load_combined_data(
            zip_path)

        # Check that the loaded mesh has the correct attributes
        self.assertEqual(loaded_mesh.vertex_count,
                         self.encoded_mesh.vertex_count)
        self.assertEqual(loaded_mesh.vertex_size,
                         self.encoded_mesh.vertex_size)
        self.assertEqual(loaded_mesh.index_count,
                         self.encoded_mesh.index_count)
    ...
    def test_get_combined_data_as_bytes(self):
        ...
        zip_path = os.path.join(
            self.temp_dir, "test_combined_data_comparison.zip")
        save_combined_data_to_zip(
            encoded_mesh=self.encoded_mesh,
            encoded_arrays=encoded_arrays,
            metadata=metadata,
        ...
        file_loaded_mesh, file_loaded_arrays, file_loaded_metadata = load_combined_data(
            zip_path)
        ...
        for name in encoded_arrays.keys():
            self.assertEqual(
                loaded_arrays[name].data, file_loaded_arrays[name].data)
