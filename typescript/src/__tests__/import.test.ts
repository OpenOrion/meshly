...
import { loadZipAsBufferGeometry } from '../index'
...
describe('Mesh import', () => {
  ...
  it('should load a mesh from a zip file', async () => {
    ...
    const geometry = await loadZipAsBufferGeometry(zipData.buffer, {
      normalize: true,
      computeNormals: true
    })
  ...
  it('should compute normals when requested', async () => {
    ...
    const geometryWithoutNormals = await loadZipAsBufferGeometry(zipData.buffer, {
      computeNormals: false
    })

    // Load the mesh from the zip file with computing normals
    const geometryWithNormals = await loadZipAsBufferGeometry(zipData.buffer, {
      computeNormals: true
    })
